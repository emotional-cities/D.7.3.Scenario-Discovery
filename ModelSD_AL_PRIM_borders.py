...
for i in range(n_iter):
    ...
    for i in range(X.shape[-1]):
        ...
        for j in range(X.shape[-1]):
            if j != i: 
                new_X_point[j] = random.choices([dimension[j,0],dimension[j,1]], k=1)[0]
    ...
    #Check tha the point choosen is not already in the dataset
    while torch.all(torch.isin(X_new_labeled, new_X_point)):
        new_X_point = random.choices(new_possible_points, k=1)[0]

    new_X_point = new_X_point.numpy()
...
with open('Output_AL_PRIM_borders100.pickle', 'wb') as handle:
    pickle.dump(output, handle, protocol=pickle.HIGHEST_PROTOCOL)
with open('X_AL_PRIM_borders100.pickle', 'wb') as handle:
    pickle.dump(X_new_labeled, handle, protocol=pickle.HIGHEST_PROTOCOL)
with open('Y_AL_PRIM_borders100.pickle', 'wb') as handle:
