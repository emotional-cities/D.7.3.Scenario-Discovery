...
def variation(x, z, params):
    ...
    y = sbx(x, z, params) #crossover
    return y

def __centroids_filename(k, dim): #create the name of the file for the centroids
    return 'centroids_new' + str(k) + '_' + str(dim) + '.dat'


def __write_centroids(centroids): #write the coordinates of the controids in a file
    k = centroids.shape[0] #number de centroids
    dim = centroids.shape[1] #dimensions of the space
    filename = __centroids_filename(k, dim) #another funtion
...
def cvt(k, dim, samples, cvt_use_cache=True): #return an array with the coordinates of the centroids
    # check if we have cached values
    fname = __centroids_filename(k, dim)
    if cvt_use_cache: #use a predefined grid
    ...
    x = np.random.uniform(0.1, 2, size=(samples, dim))
    k_means = KMeans(init='k-means++', n_clusters=k,
                     n_init=1, n_jobs=-1,  verbose=1)#,algorithm="full")
    k_means.fit(x) # x is the number of random samples used to determine the position of the centroids
    __write_centroids(k_means.cluster_centers_) #write the coordinates of the centroids in a file

    return k_means.cluster_centers_ #return the coordinates of the centroids
...
def parallel_eval(evaluate_function, to_evaluate, pool, params): #function to evaluate the fitness in parallel or not
...
def __save_archive(archive, gen):
    ...
    with open(filename, 'w') as f:
        for k in archive.values(): #write every individual in the archive
